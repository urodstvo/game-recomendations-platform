...
def analyze_comment(game_urls, n=100):
    def saveModel():
        ...
        train_base = count_idf.transform(train['text'])
        test_base = count_idf.transform(test['text'])
        model = LogisticRegression(solver='lbfgs', random_state=12345, max_iter=10000, n_jobs=-1)
        model.fit(train_base, train['label'])
    ...
    def get_proba(url):
        ...
        reviews_tf_idf = count_idf.transform(reviews_df['text_clear'])
        reviews_neg_proba = model.predict_proba(reviews_tf_idf)
    ...
    comments = pd.read_csv(os.getcwd() + path_main + 'comments_clean.csv', sep=',', header=None)
    ...
    ctf_idf = count_idf.fit(comments['text'])
    # predict_lr_base_proba = model.predict_proba(tf_idf_test_base_1)
